...
\title{Étude de couvertures de réseaux de métros, application de 
...
\begin{frame}
    ...
    \itemize{
        \item Convertir les données géographique en espace métrique
        \item Trouver les trous dans cet espace
        \item Afficher ces zones 
    }

    % \begin{figure}
    %     % \includegraphics[width=\textwidth]{filtration}
    %     \centering
    %     \caption{Carte avec pts metros vers repr avec triangles}
    % \end{figure}
...
\begin{frame}
    \frametitle{Plus en détail, reconnaitre un trou : l'homologie persistante}
    Chercher à faire des chemins, puis reduire ces chemins sans jamais le rompre  
    \begin{figure}
        \begin{tikzpicture}
            \begin{axis}[
                axis equal image,
        ...
        \end{tikzpicture}
        \centering
        \caption{Un tore contenant deux trous et un cercle contenant 0 trou}
    \end{figure}
\end{frame}
